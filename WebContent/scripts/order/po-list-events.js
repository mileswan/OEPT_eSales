 ...
 * Version: 1.0
 * Date: 2015/12/28
 * Description: Purchase orders events handlers.
 */
var POEvents = function(){
	// initialize events function for all elements.
    var initPOEvents = function() {
    	...
    	//test
    	$('#new-po-cancel').click(function(){
    		$('input[name="po[man-number]"]').val("");
    		$('input[name="po[supplier-name]"]').val("");
    		$('textarea[name="po[description]"]').val("");
    	...
    	$('button.warehouse-pick').click(function() {
    		...
    		for(var i=1;i<table.rows.length;i++){		
    			if(table.rows[i].cells[0].getElementsByTagName('input')[0].checked){
    				...
    				//for new dialog
    				$('input[name="po[warehouse-id]"]').val(id);
    				$('input[name="po[warehouse-name]"]').val(warehouse_name);
    				break;
    			}
    		}
    	});
    ...
    var addPOrder = function() {
		...
		var man_number = $('input[name="po[man-number]"]').val();
		var supplier_id = $('input[name="po[supplier-id]"]').val();
		var desc = $('textarea[name="po[description]"]').val();
		var currency_code = $('select[name="po[currency]"]').val();
//		var warehouse_id = $('input[name="po[warehouse-id]"]').val();
...
//		}
		...
		$.ajax({
            ...
            success: function (result) {	
            	...
            	if( responseJson.id ){
            		$('.alert-success span').text("采购订单创建成功！");
       	   	 	 	$('.alert-success').show();
            	}else{
            		$('.alert-danger span').text("采购订单创建失败，请重新操作！");
            		$('.alert-danger').show();
            	}
    ...
    //initialize PO list table
    var initPOTable = function () {
