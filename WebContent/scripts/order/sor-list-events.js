 ...
 * Version: 1.0
 * Date: 2016/1/7
 * Description: Sales return orders events handlers.
 */
var SOREvents = function(){
	// initialize events function for all elements.
    var initSOREvents = function() {
    	
    	$('#new-sor-confirm').click(function() {
    		addSORrder();
	    });
    	...
    	$('button.warehouse-pick').click(function() {
    		...
    		for(var i=1;i<table.rows.length;i++){		
    			if(table.rows[i].cells[0].getElementsByTagName('input')[0].checked){
    				...
    				//for new dialog
    				$('input[name="sor[warehouse-id]"]').val(id);
    				$('input[name="sor[warehouse-name]"]').val(warehouse_name);
    				break;
    			}
    		}
    	});
    	...
    	$('button.customer-pick').click(function() {
    		...
    		for(var i=1;i<table.rows.length;i++){		
    			if(table.rows[i].cells[0].getElementsByTagName('input')[0].checked){
    				...
    				//for new dialog
    				$('input[name="sor[customer-id]"]').val(id);
    				$('input[name="sor[customer-name]"]').val(customer_name);
    				break;
    			}
    		}
    	});
    }
    //Add new sales return order
    var addSORrder = function() {
    	
		var man_number = $('input[name="sor[man-number]"]').val();
		var customer_id = $('input[name="sor[customer-id]"]').val();
		var desc = $('textarea[name="sor[description]"]').val();
//		var warehouse_id = $('input[name="sor[warehouse-id]"]').val();
...
//		}
		...
		$.ajax({
            ...
            success: function (result) {	
            	...
            	if( responseJson.id ){
            		$('.alert-success span').text("销售退货订单创建成功！");
       	   	 	 	$('.alert-success').show();
            	}else{
            		$('.alert-danger span').text("销售退货订单创建失败，请重新操作！");
            		$('.alert-danger').show();
            	}
