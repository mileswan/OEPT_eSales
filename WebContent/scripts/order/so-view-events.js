 ...
 * Version: 1.0
 * Date: 2016/1/6
 * Description: Sales Orders view events handlers.
 */
var SOViewEvents = function(){
	// initialize events function for all elements.
    var initSOViewEvents = function() {
    	...
    	//Begin ship address pick
    	$('a.ship-address-pick').click(function(){
    			loadAvailShipAddr();
    	});
    	$('button.ship-address-pick').click(function() {
    		var table=document.getElementById("ship_address_pick_table");
    		var id="";
    	...
    	//End ship address pick
    	//Begin area pick
    	$('a.area-pick').click(function(){
    			loadAvailArea();
    	});
    	$('button.area-pick').click(function() {
    		var table=document.getElementById("area_pick_table");
    		var id="";
    		...
    		for(var i=1;i<table.rows.length;i++){		
    			if(table.rows[i].cells[0].getElementsByTagName('input')[0].checked){
    				...
    				id = table.rows[i].cells[1].id;
    				var addressInfo = '<div class="row static-info"><div class="col-md-5 name">\
					 省/直辖市:</div><div class="col-md-7 value">'+table.rows[i].cells[1].innerText+'</div></div> \
					 <div class="row static-info"><div class="col-md-5 name">\
					 市/区:</div><div class="col-md-7 value">'+table.rows[i].cells[2].innerText+'</div></div> \
					 <div class="row static-info"><div class="col-md-5 name">\
					 区/县:</div><div class="col-md-7 value">'+table.rows[i].cells[3].innerText+'</div></div>';
    				$('div.area-info').html(addressInfo);
    				break;
    			}
    		}
    		updateAddr(id,"area");
    		...
    		for(var i=1;i<table.rows.length;i++){		
    			if(table.rows[i].cells[0].getElementsByTagName('input')[0].checked){
    				...
    				var customerInfo = '<div class="row static-info"><div class="col-md-5 name">\
    					客户编号:</div><div class="col-md-7 value">'+table.rows[i].cells[1].innerText+'</div></div> \
    					<div class="row static-info"><div class="col-md-5 name">\
    					客户名称:</div><div class="col-md-7 value">'+table.rows[i].cells[2].innerText+'</div></div> \
    					<div class="row static-info"><div class="col-md-5 name">\
    					电子邮件:</div><div class="col-md-7 value">'+table.rows[i].cells[5].innerText+'</div></div> \
    					<div class="row static-info"><div class="col-md-5 name">\
    					客户地址:</div><div class="col-md-7 value">'+table.rows[i].cells[3].innerText+'</div></div> \
    					<div class="row static-info"><div class="col-md-5 name">\
    	...
    	//End customer pick
    	//Begin lv2 customer pick
    	$('a.customer-lv2-pick').click(function(){
    		loadAvailLv2Customer();
    	});
    	$('button.customer-lv2-pick').click(function() {
    		var table=document.getElementById("customer-lv2-pick-table");
    		var id="";
    		...
    		for(var i=1;i<table.rows.length;i++){		
    			if(table.rows[i].cells[0].getElementsByTagName('input')[0].checked){
    				...
    				var customerInfo = '<div class="row static-info"><div class="col-md-5 name">\
    					...
    					<div class="row static-info"><div class="col-md-5 name">\
    					电话号码:</div><div class="col-md-7 value">'+table.rows[i].cells[4].innerText+'</div></div>';
    				$('div.customer-lv2-info').html(customerInfo);
    				break;
    			}
    		}
    		updateOrderInfo({"customer_lv2_id":id});
    	}); 
    	//End lv2 customer pick
    	//Begin delivery warehouse pick
    	$('a.delivery-wh-pick').click(function(){
    		loadAvailWarehouse();
    	});
    	$('button.delivery-wh-pick').click(function() {
...
//    				$('div.delivery-wh-info').html(warehouseInfo);
    	...
    	//End owner pick
    	$('a.so-comment-update').click(function() {
    	...
    	$('a.order-base-update').click(function() {
    		...
    		var receipt_status = $('select[name="order[receipt_status]"]').val();
    		var receipt_ratio = $('input[name="order[receipt_ratio]"]').val();
    		var receipt_due_date = $('input[name="order[receipt_due_date]"]').val();
    	...
    	$('a.address-names-update').click(function() {
    		var ship_address = $('input[name="order[ship_address]"]').val();
    		var bill_address = $('input[name="order[bill_address]"]').val();
    		updateOrderInfo({"ship_address":ship_address,
    			"bill_address":bill_address});
    	});
    	
    	$('select[name="order[receipt_status]"]').change(function(){
    		if($(this).val() == "Overdue"){
    			$('div.receipt-due-date').show();
    		}else{
    			$('div.receipt-due-date').hide();
    		}
    	});
...
//            		$('.alert-success span').text(responseJson.objname+"已成功添加！");
//       	   	 	 	$('.alert-success').show();
       	   	 	 	loadOrderItems();
            	}else{
...
//       	   	 	 	$('.alert-success').show();
    ...
    var loadAvailWarehouse = function() {
		...
		$.ajax({
            ...
            success: function (result) {	
            	...
            	$("#delivery-wh-pick").find(".modal-body").html(tableWrapper);
    ...
    //Generate available customers list dynamically
  ...
  //Generate available lv2 customers list dynamically
    var loadAvailLv2Customer = function() {
		...
		$.ajax({
            ...
            success: function (result) {	
            	var responseJson = eval ("(" + result + ")");
            	var tableData = "";
            	var tableWrapper = '<table class="table table-striped table-bordered table-hover" id="customer-lv2-pick-table"> \
							<thead> \
							<tr>\
								<th></th>\
								<th>单位编号</th>\
            	...
            	$("#customer-lv2-pick").find(".modal-body").html(tableWrapper);
            	for(var i=0;i<responseJson.length;i++){
            		var customer = responseJson[i];
            		...
            		$("table#customer-lv2-pick-table").find("tbody").append(tableData);
                }
            	
            	var table = $('#customer-lv2-pick-table');
            	initPickTable(table);
            },
            error: function(xhr, textStatus, thrownError) {
           	 alert(thrownError);
    ...
    var loadAvailShipAddr = function() {
		...
		$.ajax({
            ...
            success: function (result) {	
            	var responseJson = eval ("(" + result + ")");
            	var tableData = "";
            	var tableWrapper = '<table class="table table-striped table-bordered table-hover" id="ship_address_pick_table"> \
					<thead> \
					<tr>\
						<th></th>\
						<th>联系人</th>\
            	...
            	$("div#ship-address-pick").find(".modal-body").html(tableWrapper);
            	for(var i=0;i<responseJson.length;i++){
            		var address = responseJson[i];
            		...
            		$("table#ship_address_pick_table").find("tbody").append(tableData);
                }
            	
            	var table = $('#ship_address_pick_table');
            	initAddrPickTable(table);
    ...
    function newAddressSubmit(){
    	...
    	$.ajax({
    		...
    		success: function(res){ 
    			...
    			if(res == "2"){
    				$('.alert-success-addr span').text("已经存在该区域，请重新选择！");
           	   	 	$('.alert-success-addr').show();
         	   	 	setTimeout(function(){
         	   	 		$('.alert-success-addr').fadeOut();
         			},2000);
  ...
  //Generate available area list dynamically
    var loadAvailArea = function() {
		...
		$.ajax({
            ...
            success: function (result) {	
            	...
            	var tableWrapper = '<div class="table-toolbar"> \
            		...
            		<table class="table table-striped table-bordered table-hover" id="area_pick_table"> \
					<thead> \
					<tr>\
						<th></th>\
						<th>省/直辖市</th>\
            	...
            	$("div#area-pick").find(".modal-body").html(tableWrapper);
            	for(var i=0;i<responseJson.length;i++){
            		var address = responseJson[i];
            		
            		tableData = '<tr> <td><input type="radio" name="area_pick" value="'+address.addressId+'"/> </td> \
            		<td id="'+address.addressId+'"> '+address.province +'</span> </td> \
            		<td> '+address.city +' </td> \
            		<td> '+address.county +' </td> \
            		</tr>';
            		
            		$("table#area_pick_table").find("tbody").append(tableData);
                }
            	
            	var table = $('#area_pick_table');
            	initAreaPickTable(table);
    ...
    //Initialize area dataTable
    var initAreaPickTable = function(table) {
    ...
    var updateOrderInfo = function(dataJson) {
		...
		$.ajax({
            ...
            data: {customer_id:dataJson.customer_id,
            	customer_lv2_id:dataJson.customer_lv2_id,
            	delivery_wh_id:dataJson.delivery_wh_id,
            	owner_id:dataJson.owner_id,
            	comment:dataJson.comment,
            	ship_addr_id:dataJson.ship_addr_id,
            	bill_addr_id:dataJson.bill_addr_id,
            	ship_address:dataJson.ship_address,
            	bill_address:dataJson.bill_address,
            	man_number:dataJson.man_number,
            	receipt_status:dataJson.receipt_status,
            	receipt_ratio:dataJson.receipt_ratio,
            	receipt_due_date:dataJson.receipt_due_date,
            	orderId:orderId},
            dataType: "text",
            success: function (result) {	
            	var responseJson = eval ("(" + result + ")");
    ...
    //generate stock out requisitions
    var generateReq = function() {
		...
		$.ajax({
            ...
            success: function (result) {	
            	var responseJson = eval ("(" + result + ")");
            	if(responseJson.objname){
            		$('.alert-success span').text("出库单已成功生成！");
       	   	 	 	$('.alert-success').show();
       	   	 	 	window.location.reload();
            	}else{
            		alert(responseJson.errmsg);
  ...
  //handle sales order history list
    ...
    //handle sales order approval history list
        			...
        			tableData = '<li class="" id="a-step-li'+(i+1)+'">\
						...
						<div class="timeline-body">\
							...
							<div class="timeline-footer">\
								<a href="/OEPT_eSales/order/so_approval_list.do" class="nav-link pull-right">\
