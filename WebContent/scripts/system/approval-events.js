	...
	var initApprovalEvents = function(){
		...
		$('input[name="checkbox"]').click(function(){
			...
			$('input[name="checkbox"]:checked').each(function(){
				$('#delectButton').removeAttr('style');
				is = 2;
			});
			if(is == 1){
				$('#delectButton').attr('style','display: none;');
			}
		});
		//删除审核规则
	...
	var submitApproval = function(){
    	...
    	$.ajax({
    		...
    		success: function(res){ 
    			if(res == 1){
    				alert("保存成功");
    				window.location.href="../system/approval_rule_list.do"; 
    		    }
    			if(res == 0){
    				alert("保存失败");
    			}
...
//			divAppr = divAppr + tableWrapper;
			...
			var tableWrapper = '<span class="unStep"></span>\
					'+apprPerNames+'('+itemValue+')\
					<input type="text" value="'+apprPerNames+'" name="apprPerNames" id="'+apprPerNamesId+'" style="display: none;">\
					<input type="text" value="'+itemValue+'" name="itemValue" id="'+itemValueId+'" style="display: none;">\
					<a href="#user-pick"  data-toggle="modal" class="btn default dark-stripe updateLoad" id="updateApprDiv'+count+'">修改 </a>\
					<a class="btn default dark-stripe clearLoad" id="'+count+'">删除 </a>\
					';
