 ...
 * Version: 1.0
 * Date: 2016/1/11
 * Description: Stock out requisition events handlers.
 */
var StockOutEvents = function(){
	// initialize events function for all elements.
    var initStockOutEvents = function() {
    	...
    	$('#new-req-cancel').click(function(){
    		...
    		$('input[name="req[delivery_date]"]').val("");
    	...
    	$('button.customer-pick').click(function() {
    		...
    		for(var i=1;i<table.rows.length;i++){		
    			if(table.rows[i].cells[0].getElementsByTagName('input')[0].checked){
    				...
    				//for new dialog
    				$('input[name="req[customer-id]"]').val(id);
    				$('input[name="req[customer-name]"]').val(customer_name);
    				break;
    			}
    		}
    	});
    ...
    var addStockOutReq = function() {
		...
		//var customer_id = $('input[name="req[customer-id]"]').val();
...
//		}
		...
		$.ajax({
            ...
            success: function (result) {	
            	...
            	if( responseJson.id ){
            		$('.alert-success span').text("出库单创建成功！");
       	   	 	 	$('.alert-success').show();
            	}else{
            		$('.alert-danger span').text("出库单创建失败，请重新操作！");
            		$('.alert-danger').show();
            	}
    ...
    //initialize Stock Out list table
    var initStockOutTable = function () {

        var table = $('#stock_out_list');
