				...
				<div class="modal fade" id="portlet-config" tabindex="-1"
					...
					<div class="modal-dialog">
						<div class="modal-content">
							...
							<div class="modal-body">确认要删除所选用户？</div>
							...
							<div class="portlet-body">
								<div class="table-toolbar">
									<div class="row">
										<div class="col-md-6">
											<div class="actions">
												<a href="<%=path%>/user/newUserSkip.do" data-toggle="modal" class="btn green">
								...
								<table class="table table-striped table-bordered table-hover"
									...
									<tbody>
										<c:forEach var="user" items="${userList}">
											...
											<tr class="odd gradeX">
												<td><input type="checkbox" name="checkbox" value="${user.userId}" class="checkboxes" id="${user.userId}" onclick="checkboxClick(${user.userId})"/>
												</td>
												<td id="${user.userId}">
												<a href="<%=path%>/user/details.do?userId=${user.userId}">${user.userName}</a></td>
												<td>
													姓名：${user.lastName}&nbsp;${user.firstName} 
													<br/>
													职位：${user.position.positionName }
												</td>
												<td>${user.email}</td>
												<td>${user.lastLogin}</td>
												<td>
													<c:if test="${user.active == true }">已激活</c:if>
													<c:if test="${user.active == false }">未激活</c:if>
												</td>
											</tr>
										</c:forEach>
									</tbody>
