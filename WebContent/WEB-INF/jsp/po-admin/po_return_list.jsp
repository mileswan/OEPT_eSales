 ...
 * Version: 1.0
 * Date: 2016/1/6
 * Description:  Purchase return orders list management page.
 */
-->
<html lang="en">
<!-- BEGIN HEAD -->
		...
		<div class="page-content">
			...
			<div class="modal fade" id="new-confirm" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
				<div class="modal-dialog">
					<div class="modal-content">
						<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
							<h4 class="modal-title">新建采购退货订单</h4>
						</div>
						<div class="modal-body">
						<div class="row">
							<div class="col-md-12">
								<label class="control-label">退货单人工编号：</label>
								<input type="text" class="form-control" placeholder="请输入订单编号…" id="por-man-number" name="por[man-number]">
								<div class="form-group">
									<label class="control-label">收货单位：<span class="required">* </span></label>
									<input type="text" name="por[supplier-id]" style="display: none;">
									<div class="input-group">
											<input type="text" class="form-control" placeholder="请选择收货单位…" name="por[supplier-name]" readonly>
								...
								<label>备注</label>
								<textarea class="form-control" rows="2"	placeholder="输入备注…" name="por[description]"></textarea>
								<div class="form-group">
									<label class="control-label">发货仓库：<span class="required">* </span></label>
									<input type="text" name="por[warehouse-id]" style="display: none;">
									<div class="input-group">
											<input type="text" class="form-control" placeholder="请选择收货仓库…" name="por[warehouse-name]" readonly>
											<span class="input-group-addon">
			...
			<div class="page-bar">
				<ul class="page-breadcrumb">
					...
					<li>
						<a href="<%=path%>/order/po_return_list.do">采购管理</a>
						<i class="fa fa-angle-right"></i>
					</li>
					<li>
			...
			<div class="row">
				<div class="col-md-12">
					<!-- BEGIN PO RETURN LIST TABLE PORTLET-->
					<div class="portlet box grey-cascade">
						<div class="portlet-title">
							<div class="caption">
								<i class="fa fa-globe"></i>采购退货单列表
							</div>
							<div class="tools">
...
<!-- 									</div> -->
							...
							<table class="table table-striped table-bordered table-hover" id="por_list">
							<thead>
							<tr>
								<th class="table-checkbox">
									<input type="checkbox" class="group-checkable" data-set="#por_list .checkboxes"/>
								</th>
								<th>
									 订单编号
								</th>
							...
							<tbody>
	                		<c:forEach var="por_order" items="${po_return_list}">
	                 		<%--用EL表达式调用list对象的属性循环输出对象的各个属性值--%> 
	                		<tr class="odd gradeX" id="${por_order.order_id}">
	                			<td>
									<input type="checkbox" class="checkboxes" value="1"/>
								</td>
	                    		<td><a href="<%=path%>/order/por_details.do?id=${por_order.order_id}">
	                    		${por_order.order_number}</a></td>
	                    		<td>
	                    			${por_order.order_manual_number} 
	                    		</td>
	                    		<td>
	                    			<b>收货单位商：</b>${por_order.supplier_name}
	                    			<br><b>发货仓库：</b>${por_order.delivery_wh_name}            		
								</td>
	                    		<td>
	                    		   	${por_order.created_date}      								
	                    		</td>
	                    		<td>
	                    			${por_order.created_by_name}
	                    		</td>
	                    		<td>
	                    			${por_order.status_value}		
	                    		</td>
	                  		</tr>	
							</c:forEach>		
							</tbody>
					...
					<!-- END PO RETURN LIST TABLE PORTLET-->
				</div>
			</div>
			<!-- END Table ROWS -->
			<!--  div class="clearfix" -->
...
<script src="../scripts/order/por-common-events.js" type="text/javascript"></script>
<script src="../scripts/order/por-list-events.js" type="text/javascript"></script>
<script>
