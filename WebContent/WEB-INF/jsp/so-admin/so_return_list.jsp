 ...
 * Version: 1.0
 * Date: 2015/1/7
 * Description:  Sales Return Orders list management page.
 */
-->
<html lang="en">
<!-- BEGIN HEAD -->
		...
		<div class="page-content">
			...
			<div class="modal fade" id="new-confirm" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
				<div class="modal-dialog">
					<div class="modal-content">
						<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
							<h4 class="modal-title">新建销售退货订单</h4>
						</div>
						<div class="modal-body">
						<div class="row">
							<div class="col-md-12">
								<label class="control-label">订单人工编号：</label>
								<input type="text" class="form-control" placeholder="请输入订单编号…" name="sor[man-number]">
								<div class="form-group">
									<label class="control-label">退货单位：<span class="required">* </span></label>
									<input type="text" name="sor[customer-id]" style="display: none;">
									<div class="input-group">
											<input type="text" class="form-control" placeholder="请选择退货单位…" name="sor[customer-name]" readonly>
								...
								<label>备注</label>
								<textarea class="form-control" rows="2"	placeholder="输入备注…" name="sor[description]"></textarea>
								<div class="form-group">
									<label class="control-label">收货仓库：<span class="required">* </span></label>
									<input type="text" name="sor[warehouse-id]" style="display: none;">
									<div class="input-group">
											<input type="text" class="form-control" placeholder="请选择收货仓库…" name="sor[warehouse-name]" readonly>
											<span class="input-group-addon">
						...
						<div class="modal-footer">
							<button type="button" id="new-sor-confirm" class="btn blue" data-dismiss="modal">确认</button>
							<button type="button" class="btn default" data-dismiss="modal">取消</button>
						</div>
					</div>
					<!-- /.modal-content -->
			...
			<div class="modal fade bs-modal-lg" id="customer-pick" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
				<div class="modal-dialog modal-lg">
					<div class="modal-content">
						<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
							<h4 class="modal-title">选择退货单位</h4>
						</div>
						<div class="modal-body">
			...
			<div class="page-bar">
				<ul class="page-breadcrumb">
					...
					<li>
						<a href="<%=path%>/order/so_return_list.do">销售管理</a>
						<i class="fa fa-angle-right"></i>
					</li>
					<li>
						<a href="<%=path%>/order/so_return_list.do">销售退货单列表</a>
					</li>
				</ul>
			</div>
			<!-- END PAGE HEADER-->
				...
				<div class="col-md-12">
					...
					<div class="portlet box grey-cascade">
						<div class="portlet-title">
							<div class="caption">
								<i class="fa fa-globe"></i>销售退货订单列表
							</div>
							<div class="tools">
						...
						<div class="portlet-body">
							...
							<table class="table table-striped table-bordered table-hover" id="sor_list">
							<thead>
							<tr>
								<th class="table-checkbox">
									<input type="checkbox" class="group-checkable" data-set="#sor_list .checkboxes"/>
								</th>
								<th>
									 订单编号
								</th>
							...
							<tbody>
	                		<c:forEach var="sales_return_order" items="${so_return_list}">
	                 		<%--用EL表达式调用list对象的属性循环输出对象的各个属性值--%> 
	                		<tr class="odd gradeX" id="${sales_return_order.order_id}">
	                			<td>
									<input type="checkbox" class="checkboxes" value="1"/>
								</td>
	                    		<td><a href="<%=path%>/order/sor_details.do?id=${sales_return_order.order_id}">
	                    		${sales_return_order.order_number}</a></td>
	                    		<td>
	                    		${sales_return_order.order_manual_number}
	                    		</td>
	                    		<td>
	                    			<b>退货单位：</b>${sales_return_order.account_name}
	                    			<br><b>收货仓库：</b>${sales_return_order.receive_wh_name}            		
								</td>
	                    		<td>
	                    			${sales_return_order.created_date}
	                    		</td>
	                    		<td>
	                    		   	${sales_return_order.created_by_name}							
	                    		</td>
	                    		<td>
	                    			${sales_return_order.status_value}	
	                    		</td>
	                  		</tr>	
							</c:forEach>		
							</tbody>
...
<script src="../scripts/order/sor-list-events.js" type="text/javascript"></script>
<script>
jQuery(document).ready(function() {    
	   ...
	   $('li.so-menu').find('ul.sub-menu').children('li:eq(1)').addClass('active');
	   CommonEvents.init();
	   SOREvents.init();
});
</script>
